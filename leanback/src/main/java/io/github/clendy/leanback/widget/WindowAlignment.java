...
class WindowAlignment {
    ...
    public static class Axis {
        ...
        private int mWindowAlignment = BaseGridView.WINDOW_ALIGN_BOTH_EDGE;
        ...
        final public void setWindowAlignmentOffsetPercent(float percent) {
            if ((percent < 0 || percent > 100)
                    && percent != BaseGridView.WINDOW_ALIGN_OFFSET_PERCENT_DISABLED) {
        ...
        final public int getSystemScrollPos(int scrollCenter, boolean isAtMin, boolean isAtMax) {
            ...
            if (!mReversedFlow) {
                ...
                if (mWindowAlignmentOffsetPercent != BaseGridView.WINDOW_ALIGN_OFFSET_PERCENT_DISABLED) {
            ...
            if (!isMinUnknown && !isMaxUnknown &&
                    (mWindowAlignment & BaseGridView.WINDOW_ALIGN_BOTH_EDGE) == BaseGridView.WINDOW_ALIGN_BOTH_EDGE) {
            ...
            if (!isMinUnknown) {
                if ((!mReversedFlow ? (mWindowAlignment & BaseGridView.WINDOW_ALIGN_LOW_EDGE) != 0
                        : (mWindowAlignment & BaseGridView.WINDOW_ALIGN_HIGH_EDGE) != 0)
            ...
            if (!isMaxUnknown) {
                if ((!mReversedFlow ? (mWindowAlignment & BaseGridView.WINDOW_ALIGN_HIGH_EDGE) != 0
                        : (mWindowAlignment & BaseGridView.WINDOW_ALIGN_LOW_EDGE) != 0)
